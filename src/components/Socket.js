...
class SocketProvider extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (
      this.state.socket &&
      this.state.socket.connector.socket.io.readyState === 'open' &&
    ...
    if (this.props.auth.isLoggedIn) {
      ...
      if (
        ...
        if (
          typeof this.state.socket.connector.channels[
            `private-user_notify_${this.props.user.id}`
          ] !== 'undefined'
        ) {
          this.privateChannel = this.state.socket.connector.channels[
            `private-user_notify_${this.props.user.id}`
          ];
          this.setState({
          ...
          this.privateChannel = this.state.socket.private(
            `user_notify_${this.props.user.id}`,
          );
          this.setState(
            {
              privateChannel: this.privateChannel,
    ...
    if (this.props.auth.isLoggedIn) {
      ...
      if (
        ...
        this.setState({
          privateChannel: this.state.socket.private(
            `user_notify_${this.props.user.id}`,
          ),
        });
      }
  ...
  connectSocket = () => {
    ...
    if (this.props.auth.isLoggedIn) {
      let connect = new Echo({
        ...
        host: `wss://${
          config.env === 'dev' ? config.dev.uri + ':6001' : config.prod.uri
        }`,
        client: io,
        auth: {
          headers: {
  ...
  _listeners = () => {
    this.state.socket.connector.socket
      .on('connect', () => {
    ...
    this.privateChannel
      .listen('.new_message', (e) => {
        ...
        let check = Object.values(
          this.props.messages.messages[e.thread_id],
        ).filter((message) => {
          console.log('LISTTTTTT', message);
          if (
            message._id === e.temp_id ||
            message._id === e.message_id ||
            e.owner_id === this.props.user.id
          ) {
        ...
        if (check.length === 0) {
          ...
          if (newMessage.system !== true)
            current.recent_message.name = newMessage.user.name;
          if (e.message_type === 1) {
      ...
      .listen('.thread_joined', (event) => {
        console.log('.thread_joined', event);
      })
      .listen('.message_purged', (event) => {
        console.log('.message_purged', event);
      })
      .listen('.call_started', (event) => {
        console.log('.call_started', event);
      })
      .listen('.thread_kicked', (event) => {
        console.log('.thread_kicked', event);
      })
      .listen('.knock_knock', (event) => {
        console.log('.knock_knock', event);
      });
  };
  render() {
    return (
